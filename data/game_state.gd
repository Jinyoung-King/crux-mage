...
const CHANGELOG := [
	{"v": "v3.74", "notes": ["[게임성] 키스톤 카드 1차 — '빌드가 다 비슷하다'를 깨려는 실험! 작동 방식을 바꾸는 빌드 정체성 카드 3종을 넣었어요: ★관통 폭풍(평타가 적을 꿰뚫고 튕김)·★장판 군주(장판 지속+적 죽은 자리에 장판)·★처형 연쇄(저체력 즉사+처치 폭발). 레벨업 드래프트에 자주 등장하니 골라서 매 판 다른 플레이를 만들어보세요. 재밌으면 더 늘립니다 — 피드백 주세요!"]},
	{"v": "v3.73", "notes": ["[게임성] 경험치·레벨업 도입! 이제 카드는 웨이브마다가 아니라 '레벨업'으로 얻습니다. 적을 잡으면 경험치가 차고(강한 적일수록↑), 레벨업할 때마다 전투가 잠깐 멈추고 카드를 고릅니다. 상단에 XP 바·레벨 표시. 잘 싸워 많이 잡을수록 빨리 큰다 — 공격적 플레이 보상! 상점·이벤트·보스는 그대로 웨이브 기반입니다. 페이스(레벨 속도)는 플레이 보며 조정할게요 — 피드백 주세요!"]},
	{"v": "v3.72", "notes": ["UI 개편 마무리 — 강화·특성·스킬·룬·패치·이벤트·저편 화면의 버튼을 도감 카드 스타일로 통일했습니다(비활성 버튼은 회색 디밍). 이제 게임 전 화면이 한 톤으로 맞춰졌어요. 색·모서리 등은 한 곳(ui_kit)에서 관리됩니다."]},
	{"v": "v3.71", "notes": ["UI 개편 — 인게임 패널·버튼 통일. 상단 버튼(배속·일시정지·재시작·캐릭터)과 일시정지·능력치 패널이 도감 카드 스타일로 정리됐고, 일시정지·능력치 창엔 콘텐츠를 감싸는 다크 라운드 카드가 깔렸어요. 결과 요약·내 빌드 패널의 버튼도 통일. 이제 홈·카드선택·하단 네비·인게임까지 한 톤으로 맞춰졌습니다."]},
	{"v": "v3.70", "notes": ["하단 메뉴(네비) 개편 — 이제 탭마다 컬러 아이콘 + 라벨이 붙고, 현재 탭은 둥근 하이라이트로 강조됩니다(강화▲·특성+·도감📖·홈⌂·스킬✦·룬◆·패치▤). 다크 바 위에 깔끔하게 정렬돼 도감 화면 톤과 통일됐어요."]},
	{"v": "v3.69", "notes": ["UI 개편 2단계 — 인게임 카드 선택 화면. 드래프트가 다크 라운드 카드 위에 올라가고, 하단 버튼(자동선택·내 카드·다시뽑기)이 도감 스타일로 통일됐어요. 카드 3장의 희귀도 테두리(게임 정보)는 그대로 유지. (다음: 인게임 패널 — 일시정지·결과·내 빌드)"]},
	{"v": "v3.68", "notes": ["UI 개편 시작 — 몹 도감의 '둥근 다크 카드' 스타일로 전 화면을 통일해 갑니다. 1단계: 홈 화면. 중앙 메뉴가 다크 라운드 카드 위에 올라가고, 속성 스테이지 버튼이 속성색 카드 버튼으로 정리됐어요. (다음: 인게임 카드 선택·패널들 순차 적용)"]},
	{"v": "v3.67", "notes": ["마룡왕이 더 무시무시해졌습니다 — 크기를 화면을 압도할 만큼 더 키우고(체력·접촉피해도↑), 보스를 감싸는 속성색 '위협 오라'(발광 헤일로)를 추가했어요. 등장할 때 화면이 크게 흔들리고 붉게 번쩍입니다. (모든 보스·중간보스에 오라 적용 — 보스 존재감↑)"]},
	{"v": "v3.66", "notes": ["마룡왕 개선 — ① 이제 보스 웨이브에 딱 한 마리만 등장합니다(증원에서 보스·중간보스는 복제 제외). ② 크기를 더 키웠어요(거신답게 화면을 압도). ③ 보스·중간보스 HP바를 큼직하게 — 몸집 비례 폭 + 검은 테두리 + 고대비 빨강으로 어떤 배경에서도 또렷이 보입니다."]},
	{"v": "v3.65", "notes": ["새 거대 보스 '마룡왕' 등장! — 보스 웨이브(10·20…)에 다른 어떤 몹보다 훨씬 거대한 흑요석 용암 거신이 나옵니다(작열 눈·용암 균열·뿔관). 기존 5속성 보스(마왕·수호마왕 등)는 이제 스테이지 중간보스로 재배치돼 더 자주 만나게 됩니다(체력은 중간보스급으로 보정). 저편 모드의 장 보스는 기존대로 속성 보스를 유지해요."]},
	{"v": "v3.64", "notes": ["원근 길은 빼고(과했어요) 대신 몹 발밑에 그림자를 넣었습니다 — 적이 '땅에 서 있는' 느낌이라 하늘에서 떨어지는 인상이 줄어듭니다. 멀수록 작은 그림자, 가까울수록 큰 그림자로 거리감과도 맞물려요. 배경 땅·지평선·먼 풍경은 그대로."]},
	{"v": "v3.63", "notes": ["성으로 이어지는 길 추가! — 화면 아래(성)에서 지평선(멀리)으로 좁아지는 원근 길이 깔렸습니다. 양쪽 수렴 모서리 + 아래로 갈수록 넓어지는 가로 단으로 '땅이 멀어지고 적이 길을 따라 성으로 다가온다'는 깊이감이 확 살아납니다(더 이상 평면·낙하 느낌이 아니에요). 바이옴마다 길 색이 어우러집니다."]},
	{"v": "v3.62", "notes": ["배경을 하늘 → 땅/지평선으로! 이제 화면 위쪽에 지평선과 먼 풍경(숲의 나무 능선·용암의 화산 산맥·사막의 모래언덕·늪의 갈대)이 있고, 그 아래로 땅이 원근감 있게 펼쳐집니다(가로줄로 멀어지는 느낌). 적이 '하늘에서 떨어지는' 게 아니라 '먼 땅에서 성으로 기어오는' 구도가 됐어요. v3.61 원근 크기(멀면 작게)와 합쳐져 다가오는 압박감을 살립니다."]},
	{"v": "v3.61", "notes": ["원근감 추가 — 적이 멀리(화면 위)에선 작게, 성으로 다가올수록 커집니다. '하늘에서 떨어지는' 느낌 대신 '땅 위를 가로질러 기어온다'는 느낌을 살리는 1차예요(크기는 성 근처에서 기존과 동일해 밸런스 유지). 다음엔 배경을 하늘→땅/길로 바꿔 더 확실히 만들 수 있어요."]},
	{"v": "v3.60", "notes": ["가시밭 가시 연출 복원! — 날카로운 가시 다발이 다시 쫙 솟구칩니다(지난번에 반대로 지웠던 걸 바로잡았어요). 밋밋하던 자연 FX는 빼고 가시가 주연입니다."]},
	{"v": "v3.59", "notes": ["[게임성] 이번 판 룰 — 매 판이 다르게! 시작할 때 무작위 '룰' 1개가 적용됩니다(상단 배너로 안내). 풍요(코인↑·적 체력↑)/광전사(적 빠름·코인↑)/응축(시그니처 충전 2배)/속전속결(적 많지만 약함)/탐욕(코인 2배·시작 체력↓). 같은 캐릭터로도 판마다 결이 달라지게 하는 1차 실험이에요(능동 입력은 안 늘렸습니다). 재밌으면 '3개 중 1개 골라 시작'으로 키울게요 — 피드백 주세요!"]},
	{"v": "v3.58", "notes": ["가시밭의 삼각 가시 솟구침 연출을 뺐습니다(피드백 반영) — 다시 자연 FX로 돌아갔어요. 둔화·속박·장판 등 효과는 그대로입니다."]},
	{"v": "v3.57", "notes": ["가시밭 가시 더 크게·또렷하게 — 가시를 반경의 절반~전부 길이로 길게, 더 빽빽이(반경 비례 최대 80개), 어두운 외곽선 + 이끼 초록 본체 + 밝은 새싹빛 끝으로 어떤 배경에서도 또렷하게 보이도록 했습니다. 솟은 뒤 더 오래 머물고, 가시를 가리던 보조 먼지 연출은 제거했어요."]},
	{"v": "v3.56", "notes": ["가시밭 연출 강화 — 가시밭을 깔면 이제 날카로운 가시 다발이 바닥에서 쫙 솟구칩니다. 뭉툭한 픽셀 더미 대신 가늘고 뾰족한 삼각 가시(끝은 초록빛 하이라이트)를 반경 가득(반경 비례 최대 60개) 빽빽이 표현했어요. 그동안 실제 스킬엔 일반 자연 FX만 떴는데, 진짜 '가시'가 보이도록 연결했습니다."]},
	{"v": "v3.55", "notes": ["[게임성] 시그니처를 '관리하는 무기'로 — 이제 시간이 아니라 적을 잡을수록 시그니처가 빨리 충전됩니다(공격적으로 몰아칠 동기). 게다가 100%에서 멈추지 않고 200%까지 '과충전'되어, 늦게 쏠수록 더 강하고 크게 터져요(금빛 게이지). '지금 작게 쏠까, 모아서 한 방?' — 매 순간 타이밍 결정이 생겼습니다. 그냥 보는 게임에서 계속 관리·조준·결정하는 게임으로 바꾸는 1차 실험이에요. 어떤지 알려주세요!"]},
	{"v": "v3.54", "notes": ["긴급 수정 — v3.51~53에서 게임 시작 후 적이 안 나오던 치명 버그를 고쳤습니다(콤보 라벨 코드의 오타로 메인 스크립트 전체가 멈췄어요). 이제 정상 작동합니다. 불편을 드려 죄송합니다!"]},
	{"v": "v3.53", "notes": ["평타 잔광 — 기본 공격에도 속성색 꼬리 잔광이 생겨 발사가 더 매끄럽고 쫀득해 보입니다(그동안 스킬 마력탄만 있었어요). 평타용은 얇고 짧게 다듬어 화면이 지저분해지지 않게 했고, 저성능 단계에선 자동 생략됩니다."]},
	{"v": "v3.52", "notes": ["연출 디테일 — ① 콤보 마일스톤(10·20…) 때 콤보 글자가 더 크게 터지고 그 색으로 화면이 번쩍여 보상감을 더했어요. ② 능동 시그니처가 충전 완료되는 순간 게이지가 톡 튀어올라 '준비됨'을 놓치지 않게 했습니다. 순수 연출이에요."]},
	{"v": "v3.51", "notes": ["연속 처치 콤보! — 짧은 시간 안에 적을 잇따라 잡으면 화면 상단에 'N 콤보!'가 팝업되고, 콤보가 쌓일수록 글자가 흰색→노랑→주황→빨강으로 뜨거워집니다(10·20콤보엔 화면도 울려요). 광역기로 몰아 잡을 때 짜릿함을 더했어요. 잠깐 끊기면 콤보는 초기화. 순수 연출이라 점수·진행엔 영향 없습니다."]},
	{"v": "v3.50", "notes": ["연출 강화 — ① 능동 시그니처 발동이 화끈해졌습니다! 마법사에서 조준점으로 빛줄기가 뻗고, 착탄점에 충격파 2겹 + 흰 섬광 + 속성색 화면 플래시 + 흔들림이 터져요(이제 '내가 한 방 날렸다'는 손맛). ② 큰 적·보스를 잡으면 폭발 충격파 2겹과 큰 섬광이 화끈하게 터집니다. 시간정지(끊김) 없이 순수 연출이라 게임플레이는 그대로예요."]},
	{"v": "v3.49", "notes": ["배경 바이옴 확장 — ① 지형 실루엣 추가: 숲엔 소나무, 사막엔 모래언덕, 용암/협곡엔 암릉, 설원엔 눈언덕, 늪/심해엔 갈대가 바닥에 깔립니다. ② 속성마다 변형 2종: 깊이 들어가면 숲→정글, 용암→화산재, 사막→붉은 협곡, 설원→빙동굴, 늪→심해로 새 풍경이 나와요(무한 모드 한 바퀴=5스테이지마다 교체, 총 10풍경). ③ 홈 화면도 이제 선택 캐릭터 속성의 바이옴 풍경을 보여줍니다."]},
	{"v": "v3.48", "notes": ["배경 바이옴 — 그동안 보랏빛 밤하늘 하나였던 배경이 스테이지 속성마다 바뀝니다! 목=🌲숲(떨어지는 잎)·화=🌋용암(떠오르는 불씨+바닥 발광)·토=🏜사막(흩날리는 모래)·금=❄️설원(내리는 눈)·수=🌿늪(떠오르는 기포+안개). 무한 모드는 10웨이브마다 다음 바이옴으로 넘어가고, 스테이지 모드는 고른 속성의 바이옴이 깔려요. 색은 부드럽게 전환됩니다(순수 연출, 성능 동일)."]},
	{"v": "v3.47", "notes": ["피가 낭자하게! — 사망 혈흔을 대폭 강화했습니다. 중심 핏웅덩이에 더해 사방으로 뻗는 스프레이 줄기 + 흩뿌려진 핏방울이 촥 터지고, 더 크고 진하게·더 오래 남았다 사라져요. 큰 적일수록 피바다(보스는 화끈하게). 성능은 그대로(연출 1회 그리기 + 팝)."]},
	{"v": "v3.46", "notes": ["타격감 — 몹을 잡으면 붉은 혈흔이 촥! 터지고 잠깐 바닥에 남았다 사라집니다. 큰 적일수록 핏자국도 크고 방울이 많이 튀어요(보스는 화끈하게). 순수 연출이라 게임플레이엔 영향 없습니다."]},
	{"v": "v3.45", "notes": ["[실험] 능동 시그니처 스킬 — 드디어 전투에 '직접 하는 것'이 생겼습니다! 자동 시전은 그대로 두고(추가 개념), 주력 속성이 주는 시그니처 한 방이 짧은 쿨로 충전돼요. 화면 아래 게이지가 차면 '탭하여 발동' — 전장을 탭하면 그 지점에 강하게 터집니다. 핵심은 자동이 못 하는 결정: 자동 스킬은 '가장 밀집한 곳'을 노리지만, 위험한 건 '마법사로 돌파해오는 적'이죠. 그걸 직접 끊는 방어 한 방이에요. 속성마다 발동이 다릅니다(불=화염 봉쇄/수=빙결 저지/목=속박 차단/금=직선 저격/토=바위 강타), 위력은 어피니티에 비례. '성장하는데 플레이가 안 바뀐다'를 능동 입력으로 깨보는 1차 검증 — 재밌는지 꼭 알려주세요!"]},
	{"v": "v3.44", "notes": ["[실험] 어피니티 가독성 — 빌드 정체성을 '결정하는 순간'에 보이도록 했습니다. 카드 선택창 제목 아래에 현재 아키타입(예: '화 전문화 · 화 70%')이 항상 뜨고, 속성 스킬 카드마다 이 카드가 빌드를 어디로 끄는지 한 줄로 표시됩니다 — '화 전문화 심화'(같은 속성 깊게) / '화+수 콤보'(반응 상대) / '수 분기'(새 속성). 전엔 '내 빌드' 화면을 따로 열어야 보였던 정보라, 전문화 vs 콤보 갈림길이 고를 때 바로 읽히도록 한 거예요. 피드백 주세요!"]},
	{"v": "v3.43", "notes": ["[실험] 원소 어피니티 — 빌드에 '정체성'이 생겼습니다. 한 속성에 투자할수록(같은 속성 스킬을 모을수록) 어피니티가 쌓여 그 속성 스킬·반응이 강해지고, 불은 어피니티가 오를수록 폭발 연출이 커집니다. '내 빌드' 화면에 속성별 어피니티 막대와 아키타입(예: 화 전문화 / 화+수 콤보)이 표시되고, 카드 드래프트도 '한 속성 깊게(전문화) vs 다른 속성으로 콤보'를 가중합니다. 매 판 다른 빌드로 가는 재미를 검증하는 1차예요 — 피드백 주세요!"]},
	{"v": "v3.42", "notes": ["반응을 오행 원리로 정렬 — 폭발형 반응은 '상극'만 두기로 하고, 모순이던 감전(금+물=금생수=상생인데 폭발형)을 제거했습니다. 증발(물극불)·빙결파쇄(흙극물)·과부하는 그대로. 금속의 올바른 폭발(금극목=속박된 적 타격)은 추후 5속성 반응을 정리할 때 도입 예정이에요."]},
	{"v": "v3.41", "notes": ["리버스(실험) 모드를 홈에서 잠시 내렸습니다. 역할을 뒤집어 봤더니 '내 캐릭터를 키워 막는' 이 게임의 핵심 재미(성장·생존·내 빌드 구경)가 빠져 어색했어요. 접는 게 아니라, 나중에 '호드(군단) 쪽 고유 재미'를 제대로 설계해서 다시 가져올 계획입니다. (그동안 본 게임에 집중)"]},
	{"v": "v3.40", "notes": ["리버스 편성에서 '보물'(공격력 0인 보너스 코인 아이템)이 후보로 뜨던 것 제거 — 실제 공격 몹만 편성됩니다."]},
	{"v": "v3.39", "notes": ["[실험] 리버스 — 방어 마법사에 광역기(유성)를 줘서 편성이 의미를 갖게 했습니다. 느리고 뭉쳐 오는 졸개는 광역에 쓸려나가고, 빠른 돌격병은 광역이 떨어지기 전에 들이닥칩니다. 이제 '몇 마리'보다 '어떤 몹'을 데려오느냐가 승패를 가릅니다(머릿수 채우기 게임 탈출). 밸런스는 계속 조정 중."]},
	{"v": "v3.38", "notes": ["목(木) 정체성 '속박' 추가 — 원소 정체성 3부작 완성. 가시밭(덩굴)에 닿은 적은 잠시 완전히 묶여 멈춥니다(둔화와 별개의 새 상태). 둔화 기반 반응(감전·파쇄)과는 분리돼 있고, 같은 적을 연속으로 영구히 묶지 못하게 재속박 쿨다운이 있습니다. 이로써 화=화상·수=둔화·목=속박·금=감전·토=파쇄 — 5속성이 각자 또렷한 역할을 갖게 됐어요."]},
	{"v": "v3.37", "notes": ["홈 '저편 진입'·'리버스' 버튼 앞에 깨져 보이던 기호(폰트에 없는 특수문자)를 제거했습니다. 리버스 모드 안 표기도 동일하게 정리."]},
