...
var _hp_bar_w := HP_BAR_W  ## 이 적의 HP바 폭(보스 크기 비례)
...
func _build_hp_bar(enemy_size: float) -> void:
	_hp_bar_w = clampf(enemy_size * 0.95, 60.0, 220.0)  # 큰 보스일수록 넓은 바(가시성)
	var bar_h := 10.0
	var top := -enemy_size / 2.0 - 22.0
	# 검은 테두리 — 어떤 배경(용암 등)에서도 또렷하게
	var border := ColorRect.new()
	border.color = Color(0, 0, 0, 0.9)
	border.size = Vector2(_hp_bar_w + 6.0, bar_h + 6.0)
	border.position = Vector2(-(_hp_bar_w + 6.0) / 2.0, top - 3.0)
	add_child(border)
	# 빈 체력 배경(짙은 적)
	var bg := ColorRect.new()
	bg.color = Color(0.28, 0.05, 0.05, 1.0)
	bg.size = Vector2(_hp_bar_w, bar_h)
	bg.position = Vector2(-_hp_bar_w / 2.0, top)
	add_child(bg)
	# 채움(밝은 적) — 줄어들며 짙은 배경이 드러남
	hp_fill = ColorRect.new()
	hp_fill.color = Color(1.0, 0.32, 0.26)
	hp_fill.size = Vector2(_hp_bar_w, bar_h)
	hp_fill.position = Vector2(-_hp_bar_w / 2.0, top)
	add_child(hp_fill)
...
func _update_visuals() -> void:
	...
	if hp_fill:
		hp_fill.size.x = _hp_bar_w * maxf(hp / max_hp, 0.0)
...
func take_damage(amount: float) -> void:
	...
	_flash()  # 피격 플래시+찌그러짐(타격감 — 시간정지 없이 로컬 연출만, 끊김 없음)
...
## 피격 반응: 밝게 번쩍이며 동시에 가로로 찌그러졌다(squash) 기본으로 복귀 → 때린 무게감.
## 플래시·찌그러짐을 한 트윈(parallel)에 묶고, 이미 연출 중이면 새로 안 만듦(연사·다발 시 트윈 churn↓).
## 거대(보스)는 과한 출렁임 방지로 약하게. ※ 시간정지(히트스톱) 없음 — 끊김 방지(v1.25·v1.82 교훈).
func _flash() -> void:
	if _flashing:
		return
	_flashing = true
	...
	tw.tween_property($Sprite2D, "scale", Vector2(sprite_scale, sprite_scale), 0.12).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
