...
var _sig_was_ready := false   # 직전 프레임 준비 여부 — 충전 완료 순간 1회 팝 알림용
const COMBO_WINDOW := 1.6     # 연속 처치 인정 시간(초, 게임시간) — 끊기면 콤보 리셋
var _combo := 0               # 현재 연속 처치 수
var _combo_left := 0.0        # 콤보 유지 남은 시간
var _combo_label: Label       # 화면 상단 "N 콤보!" 강조(처치 몰아칠 때 짜릿함)
...
func _ready() -> void:
	...
	_build_combo_label()  # 연속 처치 콤보 강조
...
func _process(delta: float) -> void:
	...
	if _combo > 0:  # 콤보 유지 시간 경과하면 리셋(끊김)
		_combo_left -= delta
		if _combo_left <= 0.0:
			_reset_combo()
...
func _build_sig_gauge() -> void:
	...
	_sig_gauge.pivot_offset = Vector2(150.0, 18.0)  # 가운데 기준 — 충전 완료 팝 스케일용
...
func _build_combo_label() -> void:
	...
	_combo_label.anchor_left = 0.0; _combo_label.anchor_right = 1.0
	_combo_label.offset_top = 300.0; _combo_label.offset_bottom = 372.0
...
func _add_combo() -> void:
	...
	_combo_label.text = "%d 콤보!" % _combo
	_combo_label.add_theme_color_override("font_color", col)
	_combo_label.modulate.a = 1.0
	var milestone: bool = _combo % 10 == 0
	_combo_label.scale = Vector2(1.7, 1.7) if milestone else Vector2(1.3, 1.3)  # 마일스톤은 더 크게 팝
	create_tween().tween_property(_combo_label, "scale", Vector2.ONE, 0.16).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	if milestone:  # 10·20·… 마일스톤: 흔들림 + 콤보색 화면 글로우(보상감)
		_add_shake(5.0)
		flash_overlay.color = Color(col.r, col.g, col.b, 0.16)
		create_tween().tween_property(flash_overlay, "color:a", 0.0, 0.45)

func _reset_combo() -> void:
	_combo = 0
	if _combo_label:
		create_tween().tween_property(_combo_label, "modulate:a", 0.0, 0.3)
...
func _start_wave(index: int) -> void:
	...
	_reset_combo()  # 새 웨이브 시작 시 콤보 초기화(웨이브 사이 드래프트로 이어지지 않게)
