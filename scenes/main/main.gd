...
var beyond_armed_idx := -1  # 저편 수동 조작: 무장(선택)된 스킬 인덱스(-1=없음). 전장 탭 시 이 스킬을 조준 시전
...
func _ready() -> void:
	...
	if GameState.game_mode == "beyond":  # 저편: 인게임 드래프트 없음 — 기본 로드아웃으로 바로 1웨이브
		wave_index = -1
...
func _rebuild_skill_icons(skills: Array) -> void:
	...
	for i in skills.size():
		...
		ic.tapped.connect(_on_skill_tapped.bind(i))  # 저편: 탭으로 무장(선택)
...
func _on_skill_hold(active: bool, idx: int) -> void:
	if GameState.game_mode == "beyond":
		return  # 저편은 탭-무장 방식(_on_skill_tapped) — 홀드 사거리 미사용(무장 링은 탭이 관리)
...
## 저편 수동 조작 — 스킬 아이콘 탭으로 무장(선택). 다시 탭하면 해제. 준비된(쿨 0) 스킬만 무장 가능.
func _on_skill_tapped(idx: int) -> void:
	if GameState.game_mode != "beyond" or game_over:
		return
	if idx < 0 or idx >= $Player.skills.size():
		return
	var s: Dictionary = $Player.skills[idx]
	if s.id == "barrier_droid" or s.cd_left > 0.0:
		return  # 지속형이거나 아직 쿨 — 무장 불가
	if beyond_armed_idx == idx:
		_disarm_beyond()  # 같은 스킬 다시 탭 = 무장 해제
		return
	beyond_armed_idx = idx
	var rng: float = SkillLib.SKILL_RANGE.get(s.id, 99999.0)
	var elem: String = SkillLib.DEFS.get(s.id, {}).get("element", "")
	_range_ring.show_range(rng, ElementLib.color(elem))  # 무장 표시 = 사거리 링 지속

func _disarm_beyond() -> void:
	beyond_armed_idx = -1
	_range_ring.hide_ring()

## 저편: 무장 안 했을 때 자동으로 쓸 '준비된 첫 스킬' 인덱스(-1=없음)
func _beyond_first_ready() -> int:
...
## 저편: 전장을 탭하면 무장 스킬(없으면 준비된 첫 스킬)을 그 지점에 시전.
func _unhandled_input(event: InputEvent) -> void:
	...
	if pos == Vector2.INF:
		return
	var idx: int = beyond_armed_idx
	if idx < 0 or idx >= $Player.skills.size() or $Player.skills[idx].cd_left > 0.0:
		idx = _beyond_first_ready()  # 무장 없으면 준비된 첫 스킬 자동 사용
	if idx < 0:
		return  # 준비된 스킬 없음
	if $Player.cast_skill_manual(idx, pos):
		_disarm_beyond()
...
func _beyond_cleared() -> void:
	...
	_asc_clear_msg = "★ 저편 클리어! — %s 여정 돌파" % " · ".join(names)
	GameState.add_coins(run_coins)
	var lvl_before: int = GameState.char_level(GameState.selected)
	GameState.add_xp(GameState.selected, wave_index + 1)
	var lvl_after: int = GameState.char_level(GameState.selected)
