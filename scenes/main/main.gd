...
var _sig_gauge: SignatureGauge  # [능동] 시그니처 충전 게이지 위젯 — scenes/ui/signature_gauge.gd
var _combo_meter: ComboMeter  # 연속 처치 콤보 위젯 — scenes/ui/combo_meter.gd
var _stats: StatsView         # 능력치 오버레이 + 빌드 요약 — scenes/ui/stats_view.gd
...
var _card_pool: CardPool  # 카드 풀·드래프트 뽑기(_ready에서 $Player 주입해 생성) — scenes/main/card_pool.gd
...
# 카드 풀·드래프트 뽑기는 CardPool(scenes/main/card_pool.gd)로 분리. _card_pool 참조.
...
# [정산 멱등] 이미 GameState에 정산한 양 — 생명주기(백그라운드)·사망·포기가 _bank_run을 공용 호출해도 이중 정산 안 되게
var _banked_coins := 0
var _banked_essence := 0
var _banked_mastery := 0
...
var _keystone_draft := false  # [키스톤-스파인] 현재 드래프트가 키스톤 전용인가(첫 시작 드래프트)
# 진화 분기 드래프트 상태
...
var _build_panel: BuildPanel  # 인게임 '내 빌드/내 카드' 뷰 + 카드 획득 이력 — scenes/ui/build_panel.gd
...
func _ready() -> void:
	...
	_sig_gauge = SignatureGauge.new()
	_sig_gauge.build($HUD)  # [능동] 시그니처 충전 게이지(scenes/ui/signature_gauge.gd)
	_combo_meter = ComboMeter.new()
	_combo_meter.build(self)  # 연속 처치 콤보 강조(scenes/ui/combo_meter.gd)
	...
	_stats = StatsView.new()
	_stats.setup(self)  # 마법사 탭 능력치 오버레이 + 빌드 요약(scenes/ui/stats_view.gd)
	...
	_build_panel.build(self)  # '내 카드' 버튼 + 획득 카드 리스트 패널(scenes/ui/build_panel.gd)
	card_select.view_cards_requested.connect(_build_panel.open)  # 선택창에서 획득 카드 보기
	...
	_open_draft(false, false, true)  # [키스톤-스파인] 첫 드래프트=키스톤 전용(빌드 정체성을 먼저 박는다)

## 시작 웨이브 도약 보정: 건너뛴 웨이브분의 무작위 유효 카드를 자동 지급(빠른 복귀)
func _grant_head_start(n: int) -> void:
	for i in n:
		var drawn := _card_pool.draw(1)
		drawn = drawn.filter(func(c): return c.keystone == "")  # 키스톤은 자동지급 제외(첫 드래프트에서 직접 고르게)
		if not drawn.is_empty():
			$Player.apply_card(drawn[0])
			_build_panel.record(drawn[0])
...
func _show_run_mod_banner() -> void:
	if _run_mod.is_empty():
		return
	var lbl := UIKit.label("이번 판 ┃ %s — %s" % [_run_mod.get("name", ""), _run_mod.get("desc", "")], 19, Color(0.7, 0.95, 1.0))
...
func _show_start_build_summary() -> void:
	...
	var lbl := UIKit.label("시작 빌드 ┃ " + text, 19, Color(1.0, 0.95, 0.7))
	lbl.add_theme_color_override("font_outline_color", Color(0, 0, 0, 0.9))
	lbl.add_theme_constant_override("outline_size", 5)
	lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
...
func _process(delta: float) -> void:
	...
	_sig_gauge.update(pl, delta, game_over)  # [능동] 시그니처 충전 게이지
	_combo_meter.tick(delta)  # 콤보 유지 시간 경과하면 리셋(끊김)
...
func _on_enemy_died(pos: Vector2, color: Color, size: float, tex: Texture2D, coins: int, kind: String) -> void:
	...
	if _wave_kind(wave_index) != "bonus":  # 붉은 혈흔: 촥 터지고 잠깐 남음(보물은 피 안 남김)
		...
		$Fx.add_child(blood)
		blood.setup(size)
		_combo_meter.add()  # 연속 처치 콤보(보물 픽업은 제외)
...
func _open_draft(rare: bool = false, legendary: bool = false, keystone_only: bool = false) -> void:
	$Player.skills_paused = true  # 드래프트 중 스킬 쿨타임 정지
	_draft_rare = rare or legendary  # 리롤도 최소 희귀+ 유지
	_keystone_draft = keystone_only  # [키스톤-스파인] 리롤도 키스톤 전용 유지용
	var n := clampi(CHOICES_PER_CLEAR + GameState.asc_choices_delta(), 2, 3)  # 카드 UI 슬롯 3개 — 하한 2·상한 3 안전 클램프
	var cards := _card_pool.draw(n, rare, [], legendary, keystone_only)
	if keystone_only:
		card_select.open(cards, [], 0, "★ 빌드를 정하라 — 키스톤 선택")
	else:
...
func _open_shop() -> void:
	...
	_shop_cards = _card_pool.draw(CHOICES_PER_CLEAR)
	_shop_cost = []
...
func _on_reroll_card(index: int) -> void:
	...
	var fresh := _card_pool.draw(1, _draft_rare, card_select.shown_cards, false, _keystone_draft)
...
func _after_pick() -> void:
	_keystone_draft = false  # 키스톤 픽 끝 — 이후 시작 드래프트/웨이브는 일반 풀
...
func _consume_skill_pick(card: CardData, cont: Callable) -> bool:
	...
	_build_panel.record(card)  # 스택 픽도 '내 카드'에 집계
...
func _stage_cleared() -> void:
	...
	_bank_run()  # 런 정산(멱등) — 사망·포기와 동일 경로
...
func _beyond_cleared() -> void:
	...
	var lvl_before: int = GameState.char_level(GameState.selected)
	_bank_run()  # 런 정산(멱등) — 사망·포기와 동일 경로(정수 포함)
	var lvl_after: int = GameState.char_level(GameState.selected)
	var leveled: String = ("%s 숙련 Lv %d" % [GameState.selected.display_name, lvl_after]) if lvl_after > lvl_before else ""
	_update_best_label()
	get_tree().paused = true
...
func _on_player_died() -> void:
	...
	var lvl_before: int = GameState.char_level(GameState.selected)
	_bank_run()  # 런 정산(코인·정수·숙련·기록·플레이수) — 멱등(생명주기에서 이미 일부 정산됐어도 안전)
...
func _try_levelup_draft() -> void:
	if _pending_levelups <= 0 or game_over:
		return
	if card_select.visible or _shop_active or _active_event != null or _evo_id != "" or _build_panel.is_open():
...
func _show_result(is_clear: bool, reached: int, leveled_name: String, new_best: bool, unlocked_tabs: Array = []) -> void:
	...
	var total_cards := _build_panel.total_count()
	var lines: Array = []
	lines.append(("%d웨이브 완주" % reached) if is_clear else ("Wave %d 도달" % reached))
	lines.append("처치 %d마리   ·   카드 %d장(%d종)" % [run_kills, total_cards, _build_panel.kind_count()])
	...
	if not $Player.relic_levels.is_empty():
		...
		for id in $Player.relic_levels:
			rnames.append("%s Lv%d" % [_stats.relic_name(id), $Player.relic_levels[id]])
...
## 런 진행분(코인·정수·숙련·최고기록·플레이수)을 GameState에 정산. 멱등 — 사망·포기·생명주기(백그라운드/뒤로가기) 공용.
## 코인·정수·숙련은 이미 정산한 양과의 '증가분'만, 최고 기록은 best-of, 플레이 수는 런당 1회 집계해 이중 정산을 막는다.
func _bank_run() -> void:
	if GameState.game_mode == "reverse":
		return  # 실험 모드 — 정산 없음
	var reached: int = wave_index + 1
	GameState.record_wave(reached)  # 최고 기록(best-of, 반복 호출 안전)
	var dc: int = run_coins - _banked_coins
	if dc > 0:
		GameState.add_coins(dc); _banked_coins = run_coins  # 코인 증가분만 정산
	if GameState.game_mode == "beyond":
		var de: int = run_essence - _banked_essence
		if de > 0:
			GameState.add_essence(de); _banked_essence = run_essence  # 정수 증가분만
	var dm: int = reached - _banked_mastery
	if dm > 0:
		GameState.add_xp(GameState.selected, dm); _banked_mastery = reached  # 숙련 증가분만(총합 = 도달 웨이브)
	if not _run_counted:
		_run_counted = true
...
func _on_pause_pressed() -> void:
	if game_over:
		return
	$HUD/PauseScreen/Center/BuildLabel.text = _stats.summary()
	get_tree().paused = true
	pause_screen.show()
