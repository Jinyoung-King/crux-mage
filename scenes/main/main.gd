...
const SPEEDS := [1.0, 2.0, 3.0, 4.0, 6.0]  # 배속 순환 단계(탭마다 1→2→3→4→6→1x). 강해진 뒤 빠른 정리용 고배속(연산은 EnemyCache로 경감)
...
## 배속 버튼: 1→2→3→4→6→1x 순환. Engine.time_scale 하나로 이동·타이머·트윈이 모두 가속됨.
