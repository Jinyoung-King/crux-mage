...
const MAX_ALIVE := 38  # 동시 생존 적 수 상한 — 가득이면 스폰 보류(후반 렉↓, 총 적 수·난이도는 유지하고 출현만 분산)
const MAX_PROJECTILES := 50  # 동시 발사체 상한 — 연사·다발 폭증 시 초과분 드랍(후반 렉↓)
const MAX_FX := 40  # 동시 FX 상한 — 가득이면 명중 스파크 등 비필수 FX 생략(후반 렉↓)
