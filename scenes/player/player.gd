...
const SIGNATURE_CD := 5.0          ## 시그니처 쿨다운(초) — 잦은 능동
const SIGNATURE_POWER_MULT := 2.5  ## 시그니처는 자동 스킬보다 강한 한 방(기본 위력 배수)
const SIGNATURE_RADIUS_MULT := 1.4 ## 범위도 넓게(eff_radius에서 MAX_SKILL_RADIUS로 클램프)
...
var signature_cd_left := 0.0       ## 시그니처 남은 쿨(0이면 발동 가능)
...
func _process(delta: float) -> void:
	...
	if signature_cd_left > 0.0:
		signature_cd_left = maxf(0.0, signature_cd_left - delta)  # [능동] 시그니처 충전
...
func signature_ready() -> bool:
	return signature_cd_left <= 0.0 and hp > 0.0
...
func cast_signature(aim: Vector2) -> bool:
	...
	var sig := {
		...
		"power": float(def.get("power", 10.0)) * SIGNATURE_POWER_MULT,
		"radius": float(def.get("radius", 100.0)) * SIGNATURE_RADIUS_MULT,
	...
	signature_cd_left = SIGNATURE_CD
	return true
