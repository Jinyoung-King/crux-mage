...
var can_reroll := false  ## 새로고침 1회 잔여(드래프트당) — 카드별 아이콘으로 소진
var _reroll_allowed := false  ## 이 드래프트가 카드별 새로고침 대상인가(일반 드래프트만; 상점·진화 제외)
var reroll_icons: Array = []  ## 카드별 새로고침 아이콘(소진 시 숨김)
...
func _render_cards(cards: Array) -> void:
	shown_cards = cards
	reroll_icons.clear()  # _style_card가 이전 아이콘(카드 자식)을 정리하므로 목록만 비움
	for i in buttons.size():
		var btn: Button = buttons[i]
		if i < cards.size():
			_style_card(btn, cards[i])
			btn.visible = true
			if _reroll_allowed and can_reroll:
...
func _add_reroll_icon(btn: Button, index: int) -> void:
	...
	ic.offset_top = 8.0
	ic.offset_right = -8.0
	ic.offset_bottom = 44.0
	ic.pressed.connect(_on_card_reroll.bind(index))
	btn.add_child(ic)
	reroll_icons.append(ic)

## 카드별 새로고침: 그 카드 한 장만 다시 뽑는다(드래프트당 1회 소진 → 모든 아이콘 숨김)
func _on_card_reroll(index: int) -> void:
	if picking or not (_reroll_allowed and can_reroll):
		return
	can_reroll = false
	_hide_reroll_icons()
	reroll_card_requested.emit(index)

func _hide_reroll_icons() -> void:
	for ic in reroll_icons:
		if is_instance_valid(ic):
			ic.hide()

## main이 새 카드를 뽑아 해당 자리 교체(팝 인). 새로고침은 이미 소진된 상태.
func replace_card(index: int, card) -> void:
	if index < 0 or index >= buttons.size() or index >= shown_cards.size():
		return
	shown_cards[index] = card
	_style_card(buttons[index], card)  # 자식 정리 → 그 카드의 ↻ 아이콘도 사라짐(소진됨)
