...
## 상점 '건너뛰기' 전용 (일반 드래프트는 카드별 ↻ 아이콘이 새로고침 담당)
func _on_reroll_pressed() -> void:
	reroll_button.disabled = true
	reroll_requested.emit()

## 카드 아래쪽 중앙에 새로고침(↻) 아이콘을 얹는다. 이미 새로고침한 카드면 비활성으로 표시.
func _add_reroll_icon(btn: Button, index: int) -> void:
	...
	ic.offset_left = -17.0
	ic.offset_right = 17.0
	ic.offset_top = -42.0
	ic.offset_bottom = -8.0
	ic.disabled = index < _rerolled.size() and _rerolled[index]
	ic.pressed.connect(_on_card_reroll.bind(index))
	btn.add_child(ic)

## 카드별 새로고침: 그 카드 한 장만 다시 뽑는다. 카드당 1회(총 3회) — 사용한 카드는 비활성.
func _on_card_reroll(index: int) -> void:
	if picking or not _reroll_allowed or index >= _rerolled.size() or _rerolled[index]:
		return
	_rerolled[index] = true  # 이 슬롯 1회 사용 → 교체 후 비활성 아이콘으로 표시
	reroll_card_requested.emit(index)

## main이 새 카드를 뽑아 해당 자리 교체(팝 인). 그 슬롯의 새로고침은 비활성 상태로 다시 표시.
func replace_card(index: int, card) -> void:
	if index < 0 or index >= buttons.size() or index >= shown_cards.size():
		return
	shown_cards[index] = card
	_style_card(buttons[index], card)  # 자식 정리(아이콘 포함) → 비활성 아이콘 다시 부착
	if _reroll_allowed:
		_add_reroll_icon(buttons[index], index)
...
func _style_card(btn: Button, card) -> void:
	...
	box.offset_bottom = -48.0 if _reroll_allowed else -10.0  # 새로고침 버튼 자리(아래쪽) 확보
