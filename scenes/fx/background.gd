...
func _draw() -> void:
	...
	# ⑤ 땅 원근 가로줄(아래로 갈수록 간격↑ — 면이 멀어져 보임)
	var lc: Color = ground_near.lightened(0.12)
	var line_col := Color(lc.r, lc.g, lc.b, 0.28)
	for i in range(1, 10):
		var f := float(i) / 10.0
		var ly := HORIZON + (H - HORIZON) * (f * f)
		draw_rect(Rect2(0.0, ly, W, 2.0), line_col)
	# ⑥ 바닥 발광(용암·늪 등)
	if _glow.a > 0.004:
		var ggh := 320.0
		var gb := 12
