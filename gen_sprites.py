...
# 거대 보스 "마룡왕"(흑요석 용암 거신) — 24x24, 좌우대칭으로 절차 생성. 다른 보스/중간보스보다 훨씬 거대(size로 키움).
def _build_giant():
    g = [["."] * 24 for _ in range(24)]
    def m(x, y, c):
        if 0 <= x < 24 and 0 <= y < 24:
            g[y][x] = c
            g[y][23 - x] = c
    # 둥근 거구(K): 행4~19, 좌여백 L(대칭)
    for (y, lx) in [(4,8),(5,6),(6,5),(7,4),(8,3),(9,3),(10,3),(11,3),
                    (12,3),(13,3),(14,4),(15,4),(16,5),(17,6),(18,7),(19,8)]:
        for x in range(lx, 24 - lx):
            m(x, y, "K")
    # 뿔 크라운(H): 어깨 위로 솟은 큰 뿔(대칭)
    for (hx, ys) in [(4, [1, 2, 3]), (8, [0, 1, 2, 3])]:
        for y in ys:
            m(hx, y, "H")
    # 큰 작열 눈(R): 행8~10, x5~7(+대칭 → 2개)
    for y in range(8, 11):
        for x in range(5, 8):
            m(x, y, "R")
    # 가슴 용암 균열 발광(G) 세로(대칭 2줄)
    for y in range(11, 17):
        m(8, y, "G")
    # 입(M) + 이빨(W): 행17~18 중앙(대칭으로 넓게)
    for x in range(7, 12):
        m(x, 17, "M")
    for x in [7, 9, 11]:
        m(x, 18, "W")
...
GIANT = _build_giant()
GIANT_PAL = {
    "K": (30, 24, 32, 255),     # 흑요석 몸통(거의 검정)
    "G": (255, 95, 30, 255),    # 가슴 용암 균열(작열)
    "R": (255, 150, 45, 255),   # 눈 작열 주황
    "H": (205, 198, 210, 255),  # 뿔 창백
    "M": (18, 10, 12, 255),     # 입 안
    "W": (255, 235, 200, 255),  # 이빨
}
...
write_png("enemy_titan.png", GIANT, GIANT_PAL)  # 거대 보스 마룡왕
